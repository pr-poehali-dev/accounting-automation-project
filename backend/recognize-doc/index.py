...
1) ПЕРВЫМ ДЕЛОМ ищи строку «Итого», «ИТОГО», «Всего к оплате», «К оплате», «Итого с НДС», «Всего», «на сумму», «ОПЛАТА», «Оплачено».
   Берёшь число РЯДОМ С ЭТИМ СЛОВОМ (справа или ниже). Это и есть amount.
2) Если есть сумма ПРОПИСЬЮ — например «Тридцать тысяч тридцать девять рублей двадцать шесть копеек» — расшифруй её и
   используй как контроль/основной источник: 30039.26.
3) Формат чисел: «30 039,26» / «30039,26» / «30 039.26» — убери пробелы, замени запятую на точку → 30039.26.
4) НИКОГДА не возвращай null если в тексте есть хоть одно число с «Итого» или сумма прописью.
5) Если несколько чисел рядом с «Итого» — выбирай то, что соответствует сумме прописью или МАКСИМАЛЬНОЕ.
...
8) ВНИМАНИЕ — банковские чеки/квитанции: документ может содержать НЕСКОЛЬКО сумм (например «Комиссия: 0 руб», «Сумма операции: 875 руб»).
   Нужна СУММА ОПЕРАЦИИ (платежа клиента), а НЕ комиссия банка. Ищи «Сумма», «Сумма операции», «Оплата», «Итого» — это главная сумма.
